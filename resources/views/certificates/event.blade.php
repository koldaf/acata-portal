...
<head>
    ...
    <style>
        ...
        .page { padding: 42px 56px; border: 10px solid #0f766e; min-height: 92vh; }
        .title { text-align: center; font-size: 34px; font-weight: bold; margin-top: 24px; color: #0f766e; }
...
<body>
    <div class="page">
        ...
        <div class="title">Event Certificate</div>
        <div class="subtitle">This certificate is awarded to</div>
        <div class="name">{{ $member->full_name }}</div>
        <div class="body-text">
            for participation in <strong>{{ $eventCertificate->event_name }}</strong>.<br>
            Event type: {{ ucfirst($eventCertificate->event_type) }}<br>
            Issued on {{ $issueDate }}.
        </div>
